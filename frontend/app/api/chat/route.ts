...
export async function POST(req: NextRequest) {
    try {
        ...
        const systemPrompt = {
            ...
            content: `You are a helpful assistant answering questions about MtechZilla, a software developmetn Company.
        ...
        const result = streamText({
            ...
            temperature: 0.1, // Lower temperature for more determinstic, factual responses        
        });

        // Log stream start
        console.log("Stream Started");

        // Return UI messages stream response for useChat compatibility
        ...
        if (error && typeof error === "object" && "status" in error) {
            const errorWithSatus = error as { status: number };

            if (errorWithSatus.status == 401) {
                return NextResponse.json({
            ...
            if (errorWithSatus.status == 429) {
                return NextResponse.json({
                    error: "OpenAI reate limit reached. Please try again later.",
